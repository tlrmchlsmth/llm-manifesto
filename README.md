...
The current implementation lives in this repo as the `j-llm-d` Python package
and CLI. The intended extracted package name is `llm-manifesto`, with
`manifesto` as the product and command name.
...
```yaml
# Generated by:
#   j-llm-d render models/qwen/aggregated.yaml --cluster clusters/oci-gb200.yaml --user tms
...
```bash
uv run j-llm-d render models/qwen/aggregated.yaml \
...
```text
jllmd/                  Python renderer implementation
...
models/deepseek-v4-gb200/decode-bench.yaml
```
...
The Justfile uses `JLLMD_CLUSTER` to choose the profile:

```bash
JLLMD_CLUSTER=clusters/oci-gb200.yaml just start models/qwen/aggregated.yaml
```
...
Compatibility wrappers such as `benchmark-constant`, `benchmark-stairs`,
`eval-gsm8k`, `eval-gpqa`, `prep-gpqa`, and `stop-nyann` still exist.
